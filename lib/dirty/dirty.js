...
Dirty.prototype._writeCompactedData = function(ws) {
    ...
    var buildBundle = function() {
      var bundleLength = 0,
          bundleStr = '';
      var compact = (typeof self._compactingFilter == "function");
      for (var i=0; i< keys.length; i++) {
        var doc = self._docs[keys[i]];
        if (compact && self._compactingFilter(keys[i],doc)) {
...
Dirty.prototype.setCompactingFilter = function(filter) {
    this._compactingFilter = filter;
};
