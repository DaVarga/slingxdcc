...
handle.xdccProgress = function (dbpack, pack, received) {
    ...
    if (pack.status == "canceled") {
        ...
        const settingsArr = sc.nw.get(`${dbpack.network}:downloads:${dbpack.bot}`);
        settingsArr.shift();
        sc.nw.set(`${dbpack.network}:downloads:${dbpack.bot}`, settingsArr);
    }
...
class SlingManager {
        ...
        async.waterfall([
            (callback)=> {
                ...
                if (!_.isNull(pack) && networks.has(pack.network)) {
                    ...
                    const settingsArr = sc.nw.get(`${pack.network}:downloads:${pack.bot}`) || [];
                    settingsArr.push({
                        pack: pack.id,
                        nick: pack.bot,
                        name: pack.name
                    });
                    sc.nw.set(`${pack.network}:downloads:${pack.bot}`, settingsArr);
    ...
    boot() {
        ...
        for (var key in jNetwork){
            ...
            console.log(JSON.stringify(irc));
        }
    }
