...
const defaults = {
    password: "",
    observed: true,
    regex: /#(\d+)\s+(\d+)x\s+\[\s*[><]?([0-9\.]+)([TGMKtgmk]?)]\s+(.*)/,
    groupOrder: [
        "id",
        "gets",
        "size",
...
class SlingChannel {
    ...
    constructor(channel, opts) {
        ...
        let password = _.isString(opts.password) ? opts.password : defaults.password,
            observed = _.isBoolean(JSON.parse(opts.observed)) ? JSON.parse(opts.observed) : defaults.observed,
            regex = _.isRegExp(opts.regex) ? opts.regex : defaults.regex,
            groupOrder = _.isArray(opts.groupOrder) ? opts.groupOrder : defaults.groupOrder;
    ...
    /**
     * Function to create an SlingChannel object from JSON
     * @param {string} json - JSON object
     * @param {string} json.channel - Channel name
     * @param {string} [json.password] - Channel password
     * @param {boolean} [json.observed=true] - enable package capturing
     * @param {RegExp} [json.regex] - regular expression for package capturing
     * @param {string[]} [json.groupOrder] - ordering of the capture groups.
     * @returns {SlingChannel} - fresh instance of SlingChannel
     * @throws Error on invalid parameters!
     * @public
     * @static
     */
    static fromJSON(json){
        if(!_.isObject(json))
            throw new Error("parameter is not from type object");
        if(!_.isString(json.channel))
            throw new Error("parameter json.channel ist not a string");

        return new SlingChannel(json.channel,json);
    }
    ...
    toJSON() {
        return {
            channel: this.name,
            observed: this.observed == defaults.observed ? undefined : this.observed,
            password: this.password == defaults.password ? undefined : this.password,
            status: this.status,
            topic: this.topic,
            regex: this.regex == defaults.regex ? undefined : this.regex.toString(),
            groupOrder: _.isEqual(this.groupOrder, defaults.groupOrder) ? undefined : this.groupOrder
        };
    }
     ...
     */
    get status() {
        return _privates.get(this).status == "" ? undefined : this.status;
    }

    /**
     * set the observed state
     * @param {string|undefined} status - channel status
     * @throws Error on invalid value!
     * @public
     */
    set status(status) {
        const privates = _privates.get(this);
        if (_.isString(status) || _.isUndefined(status)) {
