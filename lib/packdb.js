...
var packdb = function packdb() {
    ...
    var compactingFilter;
    packDb.on('load', function () {
        packDb.compact();
        packDb.addCompactingFilter(compactingFilter);
    })
    ...
    this.setCompactngFilter = function(hours){
        if(hours != false){
            compactingFilter = function(key,value){
                return (value.lastseen + hours * 1000 * 60 * 60) > new Date().getTime();
            }
        }else{
            compactingFilter = function(key,value){
                return false;
    ...
    this.setCompactngFilter(nconf.get('logger:removePacketsOlder_Xhours'));
