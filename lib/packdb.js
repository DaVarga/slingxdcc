...
var packdb = function packdb() {
    ...
    packDb.on('load',function(){
        console.log("redundantlength"+packDb.redundantLength);
        console.log("length"+packDb.length);
        packDb.compact();
    })
    ...
    this.startCronjob = function(minutes,redPercentage){
        if(cronID){
            clearTimeout(cronID);
            cronID = false;
        }
        cronID = setInterval(function(){
            if (packDb.redundantLength > packDb.length * redPercentage / 100) {
                packDb.compact();
            }
            },redPercentage*1000*60);
    }
    this.startCronjob(nconf.get('logger:cleandb_Xminutes'),nconf.get('logger:redundantPercentage'));
