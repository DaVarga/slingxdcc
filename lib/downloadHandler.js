    ...
    function createRequest(packObj) {
        ...
        requests[packObj.server][packObj.nick].connectHandler = function (pack) {
            if (pack.filename != packObj.filename) {
                ...
                self.emit('dlstart', {packObj: {
                    server: packObj.server,
                    nick: packObj.nick,
                    nr: packObj.nr,
                    realsize: packObj.realsize
                }});
        ...
        requests[packObj.server][packObj.nick].progressHandler = function (pack, recieved) {
            var progress = recieved / pack.filesize * 1000;
            if (recieved > packObj.recieved + (1024 * 1024)) {
                var lastPercent = recieved - packObj.recieved;
                var lastPercentin = Date.now() - packObj.lastProgress;

                packObj.lastProgress = Date.now();
                packObj.speed = parseInt(lastPercent / (lastPercentin / 1000));
                packObj.recieved = recieved;
                packObj.progress = parseInt(progress) / 10;
                ...
                self.emit('dlprogress', {packObj: {
                    ...
                    nr: packObj.nr,
                    speed: packObj.speed,
                    recieved : packObj.recieved,
                    progress : packObj.progress
                }});
            }
        ...
        requests[packObj.server][packObj.nick].completeHandler = function (pack) {
            self.emit('dlsuccess', {packObj: {
                server: packObj.server,
                nick: packObj.nick,
                nr: packObj.nr,
                recieved : pack.recieved,
                progress : 100
            }});
            dequeueDownload(packObj);
        }
