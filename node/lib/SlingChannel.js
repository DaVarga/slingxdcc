...
class SlingChannel {
    ...
    constructor(channel, opts) {
        ...
        let password = _.isString(opts.password) ? opts.password : defaults.password,
            observed = !_.isUndefined(opts.observed) && _.isBoolean(JSON.parse(opts.observed)) ? JSON.parse(opts.observed) : defaults.observed,
    ...
    toJSON() {
        ...
        if(this.observed != defaults.observed)
            json.observed = this.observed;
        if(this.password != defaults.password)
            json.password = this.password;
        if(this.regex != defaults.regex)
            json.regex = this.regex.toString();
        if(!_.isEqual(this.groupOrder, defaults.groupOrder))
            json.groupOrder = this.groupOrder;
        return json;
    }
